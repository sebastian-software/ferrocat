...
Same-language Rust peers are still used where they exist and are maintained: the
`polib` crate is the fairest in-process comparison for the PO parse/stringify
paths (no FFI or subprocess bias). For catalog updates, `babel`'s `Catalog.update`
and GNU `msgmerge` are the maintained tools with real merge semantics.
...
- `rust-libraries-v1`
  - in-process Rust-vs-Rust comparison; needs no Node/Python/gettext adapter setup
  - compares `ferrocat` parse and stringify against the `polib` crate across the `de`/`fr`/`pl` gettext corpora
  - `polib` only reads/writes PO, so there is no merge comparison on this axis
- `catalog-update-v1`
