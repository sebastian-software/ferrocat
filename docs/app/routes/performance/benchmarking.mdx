...
run against the established JS/Python tools (`pofile`, `gettext-parser`, Python
`polib`). For catalog updates, `babel`'s `Catalog.update` and GNU `msgmerge` are
the maintained tools with real merge semantics.
