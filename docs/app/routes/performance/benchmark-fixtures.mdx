...
description: "Reference guide to the generated and static benchmark fixture sets used in ferrocat-bench."
order: 20
---

# Benchmark Fixtures

`ferrocat-bench` uses deterministic fixture classes so performance changes can be compared across time instead of explained by changing input data.
...
- the other Gettext families stay valuable as realistic classic workload corpora even when a given third-party parser is not part of that comparison group
...
cargo run -p ferrocat-bench -- combine-catalogs catalog-modern-de-1000 200
