...
Add `merge_catalogs_three_way` for host-neutral version-control workflows.
The API receives explicit ancestor, ours, and theirs roles and applies
deletion-aware identity rules before delegating retained definitions to the
ordinary combine ownership logic. One-sided changes apply without conflict;
modify/delete and independently changed values follow
`CatalogConflictStrategy`. The `Error` strategy exposes modify/delete as a
structured `ApiError::ModifyDeleteConflict`.

Ferrocat does not infer Git state. A host maps merge or rebase roles before it
calls the API and owns any filesystem transaction around the in-memory result.
...
- version-control hosts can share one deletion-aware PO/FCL merge contract without reimplementing catalog metadata ownership
...
- hosts still need to map version-control-specific roles and decide how to write the in-memory result
