---
title: "ADR 0009: Use Versioned, Truncated SHA-256 Keys for Runtime Catalog Compilation"
description: "Accepted architecture decision record: runtime catalog compiled key strategy."
order: 90
---
...
The strict `compiled_key(msgid, msgctxt)` contract remains byte-for-byte
unchanged. Hosts that opt into `IcuSyntaxPolicy::RuntimeLiteralApostrophes` use
the additive `compiled_key_with_policy` helper. It canonicalizes only the ICU
`msgid` before applying the same `FerrocatV1` payload and hash:

- ordinary literal apostrophes are doubled
- existing ICU quotes before `{`, `}`, and plural/selectordinal `#` keep their
  meaning
- escaped `''` remains one escaped apostrophe
- unterminated runtime quotes are closed in the canonical form
- `msgctxt` remains untouched because it is a plain disambiguation string

The canonicalization is idempotent and is also exposed through
`canonicalize_icu_with_policy`. Policy-aware catalog compilation and
`CompiledCatalogIdIndex::new_with_policy` derive IDs from that same canonical
text. Selected compilation recomputes each requested ID from its mapped source
identity under the active key strategy and syntax policy, rejecting an
incompatibly derived ID instead of silently emitting a different output ID.
...
- runtime-dialect host adapters can share canonical text and IDs with catalog
  compilation without reimplementing apostrophe handling
...
- callers using policy-aware selected compilation must request IDs derived
