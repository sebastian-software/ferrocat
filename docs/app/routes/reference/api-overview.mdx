...
- `fuzzy`: active target entry carrying a visible `fuzzy` flag
...
absent entries remain visible as separate counters so hosts can choose their own
...
In FCL storage, arbitrary gettext-style custom headers are intentionally out of scope for `v1`; only the explicit `%FCL1` header metadata and per-entry columns are persisted. Machine-translation metadata uses optional `mt.*` tags appended to the entry line.
