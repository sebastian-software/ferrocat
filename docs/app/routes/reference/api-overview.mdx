...
When no mode is provided, `CatalogFileFormat::Po` uses `CatalogMode::IcuPo` and `CatalogFileFormat::Ndjson` uses `CatalogMode::IcuNdjson`. PO callers that need classic gettext plural semantics can set `mode: Some(CatalogMode::GettextPo)`.
