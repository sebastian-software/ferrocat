...
FCL catalogs store the merge-stable subset as `mt.*` tags appended to the entry line:

```fcl
Hello		Hallo	mt.model=openai/gpt-5.5-high	mt.conf=95	mt.hash=...
```

FCL deliberately omits `modified` because timestamps churn every regenerated
line and make clean git merges harder. Use the hash for freshness checks in FCL
storage; keep PO when timestamp round-tripping matters.
