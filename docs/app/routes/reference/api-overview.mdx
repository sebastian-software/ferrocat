...
Ferrocat assumes exact source-string identity here. If your project uses real product copy as `msgid`, this keeps updates predictable: matching messages keep translations, new source strings are added, and removed source strings follow the obsolete strategy. Fuzzy matching is intentionally not the default center of this workflow.
...
- keep the first translation by default, so existing translations can be listed before newer templates
