...
| Merge explicit ancestor, ours, and theirs catalogs while preserving deletions | `merge_catalogs_three_way` |
...
### `merge_catalogs_three_way`

Use this when a version-control host has an explicit common ancestor plus two current catalog states. Unlike an N-way overlay, a three-way merge can distinguish an unchanged ancestor entry from a current entry and therefore preserves intentional deletions.

The identity is `msgid` plus optional `msgctxt`. The merge follows these rules:

- an entry deleted from both current sides stays deleted
- a one-sided deletion wins when the other side still matches the ancestor
- an entry added on either current side is retained
- a one-sided modification wins without producing a conflict
- independently changed values use `CatalogConflictStrategy`
- a modify/delete conflict uses the same strategy and returns `ApiError::ModifyDeleteConflict` when the strategy is `Error`

`UseFirst` selects the logical `ours` side and `UseLast` selects `theirs`. Hosts must map Git or other version-control roles before calling Ferrocat; Ferrocat does not inspect repository state or infer whether an operation is a merge or rebase.

PO and FCL use the same entry rules. Retained definitions pass through the ordinary combine ownership logic, so extractor comments and origins cumulate while translator comments, flags, and machine metadata stay attached to the definition that owns the selected value. Obsolete entries participate in change detection and remain obsolete when retained.
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let merged = merge_catalogs_three_way(MergeCatalogsThreeWayOptions::new(
        CatalogCombineInput::labeled(
            "msgid \"Removed\"\nmsgstr \"Alt\"\n",
            "ancestor",
        ),
        CatalogCombineInput::labeled("", "ours"),
        CatalogCombineInput::labeled("", "theirs"),
        "en",
    ))?;
...
- Deletion-aware ancestor/ours/theirs merge: `merge_catalogs_three_way`
