...
| Exchange singular catalog messages with XLIFF 1.2 tools | `export_xliff` / `import_xliff` |
...
### `export_xliff` / `import_xliff`

Use this bridge when singular catalog messages need to move through an XLIFF
1.2-based translation system without introducing host-framework assumptions.

The MVP preserves identity through `msgid + msgctxt`: `msgid` is the XLIFF
`source` text, and `msgctxt` is stored in a `note` with `from="msgctxt"`.
Fuzzy translations map to review state, and empty targets map to
`needs-translation`.

Plural messages are intentionally not flattened in this first bridge. Export
reports and skips source plurals; import leaves matching existing plural
messages unchanged and returns a diagnostic. See [ADR 0019](/architecture/adr/0019-xliff-bridge-mvp)
for the exact mapping contract.
