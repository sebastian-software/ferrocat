...
| Parse and write PO files | PO is the human-editable catalog format | `pofile`, `gettext-parser`, and `polib` expose editable object models | `parse_po`, `parse_po_borrowed`, `stringify_po` | Keep expanding positioned diagnostics while preserving compact public error types |
...
| Reduce Git merge pain | Usually handled outside gettext through file layout, merge drivers, or process rules | Large teams often rely on line-based review and hosted pull request merges | ICU-native FCL stores one entry per line and keeps unchanged entries byte-identical for clean 3-way merges | Treat storage format as a collaboration feature, not just an interchange detail |
| Track AI translation metadata | Usually modeled in platform-specific sidecars or external TMS state | AI-assisted workflows often need provenance and stale-edit detection | `MachineMetadata` (integrity `lock` plus optional `AiProvenance`) on catalog entries, stored in PO or FCL | Keep an engine-agnostic integrity lock and AI provenance with the catalog, detecting by-hand edits automatically |
...
| Validate translations | `msgfmt` checks and compiles catalogs; Weblate, FormatJS, and Translate Toolkit add workflow checks | `gettext-parser` and `polib` can compile PO/MO; app stacks add their own checks | `audit_catalogs` reports ICU syntax, ICU compatibility, semantic metadata conflicts, and obsolete entries | Keep validation reports structured before considering full MO parity |
...
- **Collaboration-friendly storage.** FCL should be positioned as a practical large-team catalog format: one entry per line, cleaner pull request diffs, narrower conflicts, byte-identical unchanged entries for clean git 3-way merges, and no dependency on custom merge handlers for everyday hosted Git workflows.
- **AI-native metadata with edit detection.** Machine-managed values should carry a compact, engine-agnostic integrity lock plus optional AI provenance (model and confidence) in the catalog itself, so a later by-hand edit is detected automatically.
