...
Ferrocat is one catalog engine with a few explicit ways to store and interpret messages. You can start with the format that fits your team today and still keep a path toward richer messages, XLIFF exchange, cleaner diffs, AI translation metadata, release audits, and runtime artifacts.
...
XLIFF is not a fourth catalog mode. It is an exchange bridge for moving singular catalog messages through XLIFF 1.2 translation systems while Ferrocat keeps `msgid + msgctxt` as the source identity. Plural XLIFF mapping remains a separate policy decision so plural slots are not silently flattened.
