...
FCL removes that by construction. Every entry is one tab-separated line behind
a single `%FCL1` header. New files always use the CLDR root `(id, ctxt)` order
used by `Intl.Collator("en-US")` and record it as `order=collated` in the
header. Independent edits land on distant lines and merge cleanly; an entry
neither side touched stays byte-identical in all three versions, so ordinary
git 3-way merges preserve it. No custom merge driver required, so it behaves
the same in a hosted pull request or a web merge as it does locally. Readers
continue to accept legacy tagless FCL in its original bytewise order.
