...
Ferrocat separates two decisions that are often mixed together: how a catalog is stored and how its messages behave. Keeping those choices explicit makes migrations safer, runtime compilation clearer, and PO interoperability easier to reason about.
...
PO is the right compatibility format when you need traditional Gettext workflows and translator tooling. NDJSON solves a different collaboration problem: every message is its own line-delimited JSON record.
...
That application-centered path is where [Palamedes](/guide/palamedes) becomes important: Palamedes can own extraction, framework adapters, and runtime packaging while Ferrocat owns the PO/NDJSON catalog semantics underneath.
