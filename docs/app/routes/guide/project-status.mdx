...
public API follows semantic versioning. The reviewed `2.1.0` cleanup was a
one-time exception for removing accidental public coupling, and that window is
now closed. The focus stays product-facing: make translation catalogs easier to
update, validate, audit, compile, and ship.
...
- **Semver:** the public API follows semantic versioning. The historical
  `2.1.0` cleanup exceptions are documented in the changelog; current minor and
