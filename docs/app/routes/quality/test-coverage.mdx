...
The job publishes its evidence in three places:

- the run's job summary, which lists the measured percentage per crate next to the threshold it was checked against, including on a failing run
