...
- **AI-native metadata.** Tag any machine-written value (AI model, TMS, or a script) with a top-level integrity lock plus optional model and confidence. Edit it by hand and the lock stops matching, so machine output never ships as if a person reviewed it.
