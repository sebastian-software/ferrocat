...
/// Compile a single ICU message to a serialization-friendly representation.
///
/// For runtime formatting with host hooks, use [`crate::runtime::compile_icu`].
pub fn compile_icu(
    message: &str,
    options: &CompileIcuOptions,
) -> Result<SerializedCompiledMessage, IcuParseError> {
    let kind = compile_icu_kind(message, options)?;
    Ok(serialize_compiled_message_kind(kind))
}

/// Compile a single ICU message to a runtime evaluator.
pub fn compile_icu_runtime(
    message: &str,
    options: &CompileIcuOptions,
...
///
/// For runtime formatting with host hooks, use [`crate::runtime::compile_catalog`].
pub fn compile_catalog(
    catalog: &Catalog,
    options: &CompileCatalogOptions,
) -> Result<SerializedCompiledCatalog, IcuParseError> {
    serialize_compiled_catalog(catalog, options)
}

/// Compile a catalog to a runtime evaluator map.
pub fn compile_catalog_runtime(
    catalog: &Catalog,
    options: &CompileCatalogOptions,
) -> Result<CompiledCatalog, IcuParseError> {
    let mut messages = BTreeMap::new();

    for (catalog_key, entry) in catalog {
        let Some(translation) = &entry.translation else {
            continue;
        };

        let source_message = entry.message.as_deref().unwrap_or(catalog_key);

        let key = if options.use_message_id {
            generate_message_id(source_message, entry.context.as_deref())
        } else {
            catalog_key.clone()
        };
...
mod tests {
    ...
    use super::{
        compile_catalog, compile_catalog_runtime, compile_icu, compile_icu_runtime,
        serialize_compiled_catalog, CompileCatalogOptions, CompileIcuOptions, CompiledCatalog,
        FormatHost, MessageValue, MessageValues, SerializedCompiledMessageKind,
    };
    ...
    #[test]
    fn compile_icu_runtime_formats_literals_arguments_and_plurals() {
    ...
    #[test]
    fn compile_icu_runtime_handles_select_and_tags() {
        let compiled = compile_icu_runtime(
    ...
    #[test]
    fn compile_icu_runtime_supports_custom_host_formatters_and_locale() {
        struct TestHost;
    ...
    #[test]
    fn compile_icu_runtime_falls_back_when_not_strict() {
        let compiled = compile_icu_runtime(
            "{invalid",
            &CompileIcuOptions {
    ...
    #[test]
    fn compile_catalog_runtime_formats_messages_and_uses_keys() {
        ...
        let compiled = compile_catalog_runtime(&catalog, &CompileCatalogOptions::new("de"))
    ...
    #[test]
    fn compile_catalog_runtime_handles_gettext_plural_arrays() {
        let catalog = Catalog::from([(
    ...
    #[test]
    fn compile_icu_returns_serialized_icu_payload() {
        let compiled = compile_icu("Hallo {name}!", &CompileIcuOptions::new("de"))
    ...
    #[test]
    fn compile_catalog_returns_serializable_payload() {
        let catalog = Catalog::from([(
            String::from("Hello {name}!"),
            CatalogEntry {
                translation: Some(CatalogTranslation::Singular(String::from("Hallo {name}!"))),
        ...
        let serialized = compile_catalog(&catalog, &CompileCatalogOptions::new("de"))
            .expect("catalog should compile");
    ...
    #[test]
    fn serialize_compiled_catalog_matches_compile_catalog() {
        let catalog = Catalog::from([(
            String::from("Hello {name}!"),
            CatalogEntry {
                translation: Some(CatalogTranslation::Singular(String::from("Hallo {name}!"))),
